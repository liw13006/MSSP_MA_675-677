...
pacman::p_load(pracma,lubridate,tidyverse,dplyr,corrplot,readr,arm,readxl,coin)
``` 
...
md.corr.test <- function(md,method = "pearson"){
  y_obs <- md$model[,1]
  y_sim <- md.y_sim(md)
...
md.test <- function(md,method = "pearson",modelequation = md$call){
...
print(benchmark.ks.test <- md.ks.test(fit.benchmark))

print(benchmark.perm.test <- md.perm.test(fit.benchmark))

print(benchmark.t.test <- md.t.test(fit.benchmark))

print("pearson correlation test")
print(benchmark.corr.test <- md.corr.test(md = fit.benchmark,method = "pearson"))

print(benchmark.f.test <- md.f.test(fit.benchmark))
plot.md.hist(fit.benchmark,"y ~ MSCI(benchmark) - 1")

```

```{r, echo = TRUE}

md.test(fit.benchmark,method = "pearson",modelequation ="y ~ MSCI(benchmark) - 1" )
```
